...
async fn main() {
    ...
    let mut lbl_help = Label::new("Type a number in \neach box and click\none of the operators", 220.0, 325.0, 25);
    ...
    loop {
        ...
        if btn_add.click() {
            let num1: f64 = txt_num1.get_text().trim().parse().unwrap_or(0.0);
            let num2: f64 = txt_num2.get_text().trim().parse().unwrap_or(0.0);
